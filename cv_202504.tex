...
\tagged{GISCV}{
{\JobTitle} with extensive experience designing and implementing automated geospatial data pipelines using open-source GIS tools and software. 
Proven expertise in Python development, building GIS libraries and automation frameworks in Linux environments. 
Skilled in processing diverse geospatial formats including orthoimagery, LiDAR point clouds, elevation DEMs, and vector/raster data. 
Completed coursework in PyTorch and deep learning.
Combines strong software engineering principles with deep domain knowledge to deliver scalable, production-ready spatial solutions.
}

\iffalse
Extreamly comfortable working outside of an ArcGIS-centric workflow.
Extensive experience with Linux, *nix tools including bash.
...
• Utilize open source (e.g. GDAL) and internally developed software tools to preprocess, align, crop, resample, and otherwise prepare data for use.
• Contribute to our internal tools that process and source geospatial datasets.
• Partner with groups across the organization to understand needs for GIS data for AI/ML research.
- Coordinate with AI/ML team to provide diverse data for developing new machine learning algorithms, and the Pipeline and Product teams to provide data to roll out new regions.
...
• Experience with GIS modalities and formats, including at least some of orthoimagery (e.g. GeoTIFF), point
    clouds such as LiDAR (e.g. LAS/LAZ), vector data (e.g. ESRI Shapefiles, GeoJSON, OSM), raster data
    storing elevation and terrain information such as DTM, DSM, and DEMs, and raster data storing landcover information.
• Software engineering experience, including designing and architecting software.
- Experience in Python for automation of common tasks and/or contributing to our existing tool palette is required.
...
%                Apache Airflow workflow Orchestration: DAG creation and deployment \linebreak
    ...
    \begin{taggedblock}{GISCV}
        \begin{skillslist}
            GIS Applications:&
                QGIS, Geoserver, PostgreSQL/PostGIS \linebreak
%                - including Spatial Analyst, 3D Analyst, and Network Analyst extensions \linebreak
                \\
%            Geodatabase management:&
%                PostgreSQL/PostGIS, ESRI Enterprise, AGOL \linebreak
...
%                \\
            Programming \& Scripting:&
                \impt{Python}: ArcPy, GeoPandas, Shapely, Rasterio \linebreak
                \impt{SQL}: spatial queries, stored procedures, and functions \linebreak
                REST API development with FastAPI, Flask \linebreak
%                Experience with \impt{machine learning} with PyTorch \linebreak
%                ETL Pipelines with Python, FME \linebreak
%                Orchestration with Airflow \linebreak
%                R programming and R Studio \linebreak
%                Spatial \impt{ETL Pipelines}: FME Flow \& FME Form \linebreak
%                Data Manipulation: Power Query, DAX \linebreak
...
%                PostgreSQL - Spatial analysis, geostatistics, triggers, functions \linebreak
                ...
                Unit testing with PyTest \linebreak
...
\begin{jobtasklist}
    \item Managing spatial data pipelines at a major electric utility moving
            \impt{millions of features daily}, ensuring high availability and
            data quality through centralized orchestration and automated alerts.
    \item Architected and deployed Python library to automate the testing of spatial data pipelines
            resulting in faster testing and faster identification of pipeline issues.
\end{jobtasklist}
...
%    \item Designed and implemented robust spatial ETL pipelines with Python, and ArcPy.
%            Integrated diverse geospatial datasets and to streamline data flow,
%            significantly \impt{enhancing data reliability} and reducing manual intervention.
    \item Designed and implemented automated, open-source
        (Python/GeoPandas/GDAL/Fiona) \impt{spatial ETL pipelines}
            to preprocess, align, and reformat streaming vector data,
            ensuring data quality and reliability for critical downstream analytics applications.
...
%            improved \impt{data accessibility for critical infrastructure design}.
    \item Led the development and deployment of a cloud-based API infrastructure (Python/FastAPI on Azure),
            serving as the foundation for 19
            fully-cataloged spatial data services to enhance data accessibility and data governance.
%            delivering \impt{19 data analysis, reporting, and data management services}, providing critical business insights
%            and improving data accessibility for telecommunications infrastructure design.
%    \item Developed Python-based data loading tools (Jupyter Notebooks, GeoPandas, Rasterio, Folium) for the spatial data warehouse,
%            integrating \impt{automated quality assurance procedures} to ensure data accuracy and accelerate reliable data integration.
    \item Developed Python-based data management tools (GeoPandas, Rasterio, Folium)
            for corporate spatial-data warehouse,
            dramatically improving \impt{time to integrate and verify new data sources} and configure GeoServer.
...
%            corporate mapping platform comprising more than 7000 lines of code
    \item Designed and implemented an automated, containerized spatial ETL pipeline on Azure,
            ...
            systematically identifying and resolving data anomalies across over 13 million geospatial features daily
            to ensure data integrity for complex scientific
            applications built in Python with GeoPandas, Fiona, Rasterio, and Docker.
...
%    \item Facilitated \impt{end-user usability testing} with both technical and non-GIS stakeholders,
%        effectively translating complex geospatial concepts into actionable feedback for platform improvements and acting
%        as a critical bridge between departments.
%    \item Managed ETL pipeline and API documentation in Confluence along with support requests in Jira
    \item Sourced and aggregated publicly available LiDAR point clouds from disparate public domain sources,
            building Python tools utilizing GDAL and Rasterio to clean, reformat,
            and interpolate the data into highly accurate Digital Elevation Models (DEMs);
            dramatically improved data coverage and availability for critical infrastructure design.
\end{jobtasklist}
...
%                customer engagement and product utilization.
        ...
        \item Automated the end-to-end process for meticulous data
                \impt{verification, cleaning, and consolidation} using Python,
...
%%            problems.
%    \item Conducted \impt{advanced geospatial data modeling} and analysis on complex datasets,
%            to uncover strategic insights for public policy initiatives,
%            enabling data-driven decision-making for stakeholders tackling real-world transportation problems.
